    ...
    def list_for_user(self, user_id: int, limit: int = 10) -> List[Notification]:
        ...
        if user_id == 1: # Si c'est l'admin
             fake_notifications.append(
                Notification(id=4, user_id=1, type="system", title="API Météo", message="Le service météo a répondu avec une erreur 503.", level="critical", is_read=False, created_at=now - timedelta(minutes=30))
             )
